...
export const togglePhotoLike =
    (options = {image: {}}, cb) =>
    (dispatch, getState) => {
        api.toggleImageLike(getState().user.accessToken, {
            image: options.image
        }).then(({json, rate}) => {
...
export const toggleCollection =
    (options = {collection_id: null, photo_id: null, inCollection: null}, cb) =>
    (dispatch, getState) => {
        api.toggleAddToCollection(getState().user.accessToken, {
            collection_id: options.collection_id,
            photo_id: options.photo_id,
            collected: options.inCollection
        }).then(({json, rate}) => {
            dispatch({type: types.UPDATE_RATE_LIMIT, rate})
            return cb({json})
        })
